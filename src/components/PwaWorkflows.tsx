...
export function FeedbackForm({ onClose, onSubmitted }: { onClose: () => void; onSubmitted: (message: string) => void }) {
  ...
  return <div className="sheet-backdrop"><form className="sheet space-y-4" onSubmit={submit}><div className="flex items-start justify-between"><div><p className="eyebrow">Account</p><h2 className="mt-1 font-serif text-2xl">Feedback</h2></div><button type="button" className="icon-button" onClick={onClose} aria-label="Close"><X className="size-4" /></button></div><label className="field-label"><span className="text-destructive mr-0.5">*</span>What would you like to share?<textarea name="feedback" required rows={5} className="field-input" placeholder="Tell us what would make the workflow better..." /></label><button className="button-primary w-full"><Send className="size-4" /> Submit feedback</button></form></div>
}
