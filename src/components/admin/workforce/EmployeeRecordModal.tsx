    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-700/70 p-4" role="dialog" aria-modal="true" aria-label="New employee record">
      <div className="w-full max-w-md overflow-hidden rounded-lg bg-card shadow-2xl">
        ...
        <div className="space-y-4 px-6 py-6">
          <div className="grid grid-cols-2 gap-4"><label className="text-[0.65rem] font-bold uppercase tracking-[0.1em] text-foreground"><span className="text-destructive mr-0.5">*</span>First Name<input className={inputClass} value={draft.firstName} onChange={(e) => setDraft((p) => ({ ...p, firstName: e.target.value }))} placeholder="Lucia" /></label><label className="text-[0.65rem] font-bold uppercase tracking-[0.1em] text-foreground"><span className="text-destructive mr-0.5">*</span>Surname<input className={inputClass} value={draft.surname} onChange={(e) => setDraft((p) => ({ ...p, surname: e.target.value }))} placeholder="Mendes" /></label></div>
          <label className="block text-[0.65rem] font-bold uppercase tracking-[0.1em] text-foreground"><span className="text-destructive mr-0.5">*</span>Contact Number<input className={inputClass} value={draft.contact} onChange={(e) => setDraft((p) => ({ ...p, contact: e.target.value.replace(/\D/g, '').slice(0, 11) }))} placeholder="09123456789" inputMode="numeric" /></label>
          <label className="block text-[0.65rem] font-bold uppercase tracking-[0.1em] text-foreground"><span className="text-destructive mr-0.5">*</span>Employment Type<select className={`${inputClass} appearance-none`} value={draft.employmentType} onChange={(e) => setDraft((p) => ({ ...p, employmentType: e.target.value as EmploymentType }))}><option value="On-call">On-call</option><option value="Seasonal">Seasonal</option></select></label>
