...
- **Tools tab (activeTool is local only):** The Tools sidebar tab (Select, Draw, Shapes, Lines, Sticky Note, Text) maintains an `activeTool` state that is scoped entirely within the tab component. It is never lifted to the canvas or passed to `KonvaInfiniteCanvas`. As a result, no tool in this tab actually modifies the canvas — freehand drawing, shape placement, and line drawing are all placeholders.
...
*   **Sidebar audit [COMPLETED — partial]:** Documented functional status of all 6 sidebar tabs. Elements and Uploads are fully functional end-to-end. Background tab "Apply background" is now wired to update artboard fill color/photo across all pages (with `localStorage` persistence per project keyed by `card.id`) — implementation complete, TypeScript-clean, browser screenshot pending. Text tool buttons, Tools tab `activeTool`, and Projects sub-page navigation remain non-functional placeholders — tracked in §4 "Backlog / Future Considerations".
